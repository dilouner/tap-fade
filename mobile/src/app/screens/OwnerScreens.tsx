...
export function InviteBarberScreen({ route }: NativeStackScreenProps<RootStackParamList, 'InviteBarber'>) {
  const { data, profile } = useAppData(); const barber = data.barbers.find((item) => item.id === route.params.barberId); const [code, setCode] = useState<string | null>(null); if (!barber || !data.activeShop || !profile) return null;
  async function create() { const result = await createBarberInvite(data.activeShop!.id, barber!.id, profile!.uid); setCode(result.code); }
  return <Screen eyebrow="Equipo" title={`Invitar a ${barber.displayName}`}><PremiumHero eyebrow="Código seguro" subtitle="Vence en siete días y solo puede utilizarse una vez." title={code ?? 'Genera un código para vincular su cuenta.'} /><PrimaryButton label={code ? 'Compartir código' : 'Generar código'} onPress={() => code ? void Share.share({ message: `Únete a ${data.activeShop!.name} en TapFade con el código ${code}` }) : void create()} /></Screen>;
}
