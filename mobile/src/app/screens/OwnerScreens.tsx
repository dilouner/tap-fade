...
import { resolveBarberImage } from '../../shared/assets/demoAssets';
...
export function ServiceEditScreen({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'ServiceEdit'>) {
  ...
  const [name, setName] = useState(service?.name ?? ''); const [price, setPrice] = useState(String(service?.price ?? 250)); const [duration, setDuration] = useState(String(service?.durationMinutes ?? 45)); const [active, setActive] = useState(service?.active ?? true); const [message, setMessage] = useState<string | null>(null); const [saving, setSaving] = useState(false);
  async function save() { if (!data.activeShop || saving) return; const input = { barberShopId: data.activeShop.id, durationMinutes: Number(duration), name: name.trim(), price: Number(price) }; if (!isValidService(input)) { setMessage('Revisa nombre, precio y duración.'); return; } setSaving(true); setMessage(null); try { if (service) await updateShopService({ ...service, ...input, active }); else await createShopService(input); await refresh(); navigation.goBack(); } catch (caught) { setMessage(caught instanceof Error ? caught.message : 'No fue posible guardar el servicio.'); } finally { setSaving(false); } }
  return <Screen eyebrow="Negocio" title={service ? 'Editar servicio' : 'Nuevo servicio'}><Field label="Nombre" onChangeText={setName} value={name} /><Field keyboardType="decimal-pad" label="Precio MXN" onChangeText={setPrice} value={price} /><SelectField label="Duración" onChange={setDuration} options={['15','30','45','60','90','120','180'].map((value) => ({ label: `${value} min`, value }))} value={duration} /><SelectField label="Estado" onChange={(value) => setActive(value === 'active')} options={[{ label: 'Activo', value: 'active' }, { label: 'Pausado', value: 'paused' }]} value={active ? 'active' : 'paused'} />{message ? <Banner message={message} tone="danger" /> : null}<PrimaryButton label="Guardar servicio" loading={saving} onPress={() => void save()} /></Screen>;
}

export function BarberEditScreen({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'BarberEdit'>) {
  const { data, refresh } = useAppData(); const barber = data.barbers.find((item) => item.id === route.params?.barberId); const [name, setName] = useState(barber?.displayName ?? ''); const [specialties, setSpecialties] = useState(barber?.specialties.join(', ') ?? ''); const [serviceIds, setServiceIds] = useState(barber?.serviceIds ?? []); const [active, setActive] = useState(barber?.active ?? true); const [message, setMessage] = useState<string | null>(null); const [saving, setSaving] = useState(false);
  async function save() { if (!data.activeShop || saving) return; const input = { barberShopId: data.activeShop.id, displayName: name.trim(), serviceIds, specialties: specialties.split(',').map((item) => item.trim()).filter(Boolean), userId: barber?.userId ?? null }; if (!isValidBarber(input)) { setMessage('Escribe un nombre válido y selecciona sus servicios.'); return; } setSaving(true); setMessage(null); try { if (barber) await updateShopBarber({ ...barber, active, ...input }); else await createShopBarber(input); await refresh(); navigation.goBack(); } catch (caught) { setMessage(caught instanceof Error ? caught.message : 'No fue posible guardar el barbero.'); } finally { setSaving(false); } }
  function toggleService(id: string) { setServiceIds((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]); }
  return <Screen eyebrow="Equipo" title={barber ? 'Editar barbero' : 'Nuevo barbero'}><Field label="Nombre" onChangeText={setName} value={name} /><Field label="Especialidades" onChangeText={setSpecialties} placeholder="Fade, barba, clásico" value={specialties} /><Text style={styles.fieldLabel}>Servicios que realiza</Text><View style={styles.chips}>{data.services.map((item) => <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: serviceIds.includes(item.id) }} key={item.id} onPress={() => toggleService(item.id)} style={[styles.chip, serviceIds.includes(item.id) && styles.chipActive]}><Text style={[styles.chipText, serviceIds.includes(item.id) && styles.chipTextActive]}>{item.name}</Text></Pressable>)}</View><SelectField label="Estado" onChange={(value) => setActive(value === 'active')} options={[{ label: 'Activo', value: 'active' }, { label: 'Pausado', value: 'paused' }]} value={active ? 'active' : 'paused'} />{message ? <Banner message={message} tone="danger" /> : null}<PrimaryButton label="Guardar barbero" loading={saving} onPress={() => void save()} />{barber ? <PrimaryButton label="Crear código de invitación" onPress={() => navigation.navigate('InviteBarber', { barberId: barber.id })} variant="secondary" /> : null}</Screen>;
}
